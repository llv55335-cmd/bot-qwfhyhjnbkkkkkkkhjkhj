...
class VecStore:
    ...
    def __init__(self, conn: sqlite3.Connection):
        try:
            ...
            print(f"⚠️  sqlite-vec 不可用，降级为纯 token 检索：{e}")
...
class GraphStore:
    ...
    def create_or_update_memory_version(
        ...
        try:
            ...
            print(f"⚠️  embedding 生成失败（不影响存储）：{e}")
        ...
        with self._connect() as conn:
            ...
            cur = conn.execute(
                """INSERT INTO memories (
                    ...
                    decay_score, access_count, status, embedding_id
    ...
    def add_or_replace_path(self, domain: str, path: str, edge_id: int) -> None:
        with self._connect() as conn:
            conn.execute(
                "INSERT OR REPLACE INTO paths(domain, path, edge_id, created_at) "
                "VALUES (?, ?, ?, ?)",
                (domain, path, int(edge_id), _now_iso()),
            )
