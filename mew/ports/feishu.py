...
async def handle_token_command(chat_id: str):
    try:
        ...
        await send_to_feishu(chat_id, f"❌ 获取 token 统计失败: {str(e)}")
...
def handle_message(data):
    ...
    async def handle_schedules_command(chat_id: str):
        ...
        try:
            ...
            if not schedules:
                await send_to_feishu(chat_id, "📅 没有待提醒的日程～")
                return
            lines = ["📅 当前日程：", "================"]
            ...
            await send_to_feishu(chat_id, f"❌ 获取日程失败: {str(e)}")
